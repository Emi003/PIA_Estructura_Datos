        ...
        try:
            ...
            while True:
                ...
                if opcion == "1":
                    ...
                    if not clientes:
                        print("No hay clientes registrados o están cancelados")
                        break
                    ...
                    mi_cursor.execute("SELECT Estado_Cliente FROM Clientes WHERE ClaveC = ? AND Estado_Cliente = 0", (clave_cliente,))
                    estado_cliente = mi_cursor.fetchone()

                    if estado_cliente and estado_cliente[0] == 0:
                        ...
                        while True:
                            mi_cursor.execute("SELECT ClaveS, Nombre, Costo FROM Servicios WHERE Estado_Servicio = 0")
                            servicios = mi_cursor.fetchall()

                            if not servicios:
                                print("No hay Servicios registrados o están cancelados")
                                break
                            ...
                            mi_cursor.execute("SELECT Estado_Servicio, Costo FROM Servicios WHERE ClaveS = ?", (clave_servicio,))
                            servicio_info = mi_cursor.fetchone()

                            if servicio_info and servicio_info[0] == 0:
                                servicio_seleccionado = (clave_servicio, servicio_info[1])
                                detalles_nota.append(servicio_seleccionado)
                            else:
                                print("Error: El servicio no está activo. No se puede agregar a la nota.")
                                continue

                            agregar_otro = input("¿Desea agregar otro servicio a la nota? (SI/NO): ").strip().upper()
                            if agregar_otro != "SI":
                                break
                        ...
                        if estado_cliente and estado_cliente[0] == 0:
                            ...
                            conn.commit()
                            print("La nota ha sido registrada con éxito.")

                            mi_cursor.execute("SELECT last_insert_rowid()")
                            last_id = mi_cursor.fetchone()[0]

                            for clave_servicio, monto in detalles_nota:
                                datos_detalle = (last_id, clave_servicio, monto)
                            ...
                            print("Error: El cliente no está activo. No se puede registrar la nota.")
        ...
        while True:
            ...
            if opcion == "1":
                ...
                try:
                    with sqlite3.connect("PIA.db") as conn:
                        ...
                        if not clientes_activos:
                            ...
                            print("Clientes activos disponibles para suspender:")
                            for clave, nombre in clientes_activos:
                                print(f"Clave: {clave}, Nombre Completo: {nombre}")

                            clave_suspender = input("Ingrese la clave del cliente a suspender (o 0 para cancelar): ")

                            if clave_suspender != "0":
                                mi_cursor.execute("SELECT * FROM Clientes WHERE ClaveC = ? AND Estado_Cliente = 0", (clave_suspender,))
                                cliente_suspender = mi_cursor.fetchone()

                                if cliente_suspender:
                                    clave_suspender, nombre_suspender, rfc_suspender, correo_suspender, estado_suspender = cliente_suspender
                                    print(f"\nDetalle del Cliente a suspender (Clave: {clave_suspender}, Nombre: {nombre_suspender}, RFC: {rfc_suspender}, Correo: {nombre_suspender})")
                                    confirmar_suspension = input("¿Está seguro de suspender este Cliente? (SI/NO): ")

                                    if confirmar_suspension.upper() == "SI":
                                        mi_cursor.execute("UPDATE Clientes SET Estado_Cliente = 1 WHERE ClaveC = ?", (clave_suspender,))
                                        conn.commit()
                                        print(f"El Cliente con clave {clave_suspender} ha sido suspendido.")
                                    else:
                                        print("Suspensión cancelada.")
                ...
                try:
                    with sqlite3.connect("PIA.db") as conn:
                        mi_cursor = conn.cursor()
                        mi_cursor.execute("SELECT ClaveC, NombreCompleto FROM Clientes WHERE Estado_Cliente = 1")
                        clientes_suspendidos = mi_cursor.fetchall()

                        if not clientes_suspendidos:
                            print("No hay clientes suspendidos para reactivar.")
                        else:
                            print("Clientes suspendidos disponibles para reactivar:")
                            for clave, nombre in clientes_suspendidos:
                                print(f"Clave: {clave}, Nombre Completo: {nombre}")

                            clave_reactivar = input("Ingrese la clave del cliente a reactivar (o 0 para cancelar): ")

                            if clave_reactivar != "0":
                                mi_cursor.execute("SELECT * FROM Clientes WHERE ClaveC = ? AND Estado_Cliente = 1", (clave_reactivar,))
                                cliente_reactivar = mi_cursor.fetchone()

                                if cliente_reactivar:
                                    clave_reactivar, nombre_recuperar, rfc_recuperar, correo_recuperar, estado_recuperar = cliente_reactivar
                                    print(f"\nDetalle del Cliente a recuperar (Clave: {clave_reactivar}, Nombre: {nombre_recuperar}, RFC: {rfc_recuperar}, Correo: {correo_recuperar})")
                                    confirmar_recuperacion = input("¿Está seguro de recuperar este Cliente? (SI/NO): ")

                                    if confirmar_recuperacion.upper() == "SI":
                                        mi_cursor.execute("UPDATE Clientes SET Estado_Cliente = 0 WHERE ClaveC = ?", (clave_reactivar,))
                                        conn.commit()
                                        print(f"El cliente con clave {clave_reactivar} ha sido recuperado.")
                                    else:
                ...
                try:
                    with sqlite3.connect("PIA.db") as conn:
                        ...
                        while True:
                            ...
                            if sub_opcion == "1":
                                mi_cursor.execute("SELECT ClaveC, NombreCompleto FROM Clientes WHERE Estado_Cliente = 0 ORDER BY ClaveC")
                                clientes = mi_cursor.fetchall()
                                if not clientes:
                                    print("No se encuentra clientes registrados o activados")
                                clave_buscar = input("Ingrese la clave del cliente a consultar: ")
                                mi_cursor.execute("SELECT ClaveC, NombreCompleto, RFC, CORREO FROM Clientes WHERE ClaveC = ? AND Estado_Cliente = 0", (clave_buscar,))
                                ...
                                mi_cursor.execute("SELECT ClaveC, NombreCompleto, RFC, CORREO FROM Clientes WHERE UPPER(NombreCompleto) = UPPER(?) AND Estado_Cliente = 0", (nombre_buscar,))
                                ...
                                if not cliente:
                                    ...
                                    for ClaveC, NombreCompleto, RFC, CORREO in cliente:
                                ...
                                try:
                                    ...
                                    if ordenamiento == "1":
                                            ...
                                            if Resultado.upper() == "SI":
                                                ...
                                                if exportar.upper() == "A":
                                                    ...
                                                    try:
                                                        ...
                                                        df.to_excel(nombre_archivo, sheet_name='Sheet1', index=False)
                                                        print(f"Resultado exportado a {nombre_archivo}")
                                        ...
                                        mi_cursor.execute("SELECT NombreCompleto, ClaveC FROM Clientes WHERE Estado_Cliente = 0 ORDER BY NombreCompleto")
                                        clientes = mi_cursor.fetchall()
                                                ...
                                                if exportar.upper() == "A":
                                                    ...
                                                    try:
                                                        ...
                                                        with pd.ExcelWriter(nombre_archivo, engine='xlsxwriter') as writer:
                                                            ...
                                                            worksheet.set_column('A:A', len(max(df['Nombre'], key=len)) + 2)
                ...
                while True:
                    ...
                    if opcion == "1":
                        ...
                        try:
                            with sqlite3.connect("PIA.db") as conn:
                                ...
                                if not servicios_activos:
                                    ...
                                    print("Servicios activos disponibles para suspender:")
                                    for clave, nombre in servicios_activos:
                                        print(f"Clave: {clave}, Nombre: {nombre}")

                                    clave_suspender = input("Ingrese la clave del servicio a suspender (o 0 para cancelar): ")

                                    if clave_suspender != "0":
                                        mi_cursor.execute("SELECT * FROM Servicios WHERE ClaveS = ? AND Estado_Servicio = 0", (clave_suspender,))
                                        servicio_suspender = mi_cursor.fetchone()

                                        if servicio_suspender:
                                            clave_suspender, nombre_suspender, costo_suspender, estado_suspender = servicio_suspender
                                            print(f"\nDetalle del servicio a suspender (Clave: {clave_suspender}, Nombre: {nombre_suspender}, Costo: {costo_suspender})")
                                            confirmar_suspension = input("¿Está seguro de suspender este servicio? (SI/NO): ")

                                            if confirmar_suspension.upper() == "SI":
                                                mi_cursor.execute("UPDATE Servicios SET Estado_Servicio = 1 WHERE ClaveS = ?", (clave_suspender,))
                                                conn.commit()
                                                print(f"El servicio con clave {clave_suspender} ha sido suspendido.")
                                            else:
                                                print("Suspensión cancelada.")
                                        else:
                                            print("La clave de servicio no se encuentra en el sistema.")
                        ...
                        try:
                            with sqlite3.connect("PIA.db") as conn:
                                mi_cursor = conn.cursor()
                                mi_cursor.execute("SELECT ClaveS, Nombre FROM Servicios WHERE Estado_Servicio = 1")
                                servicios_suspendidos = mi_cursor.fetchall()

                                if not servicios_suspendidos:
                                    print("No hay servicios suspendidos para recuperar.")
                                else:
                                    print("Servicios suspendidos disponibles para recuperar:")
                                    for clave, nombre in servicios_suspendidos:
                                        print(f"Clave: {clave}, Nombre: {nombre}")

                                    clave_recuperar = input("Ingrese la clave del servicio a recuperar (o 0 para cancelar): ")

                                    if clave_recuperar != "0":
                                        mi_cursor.execute("SELECT * FROM Servicios WHERE ClaveS = ? AND Estado_Servicio = 1", (clave_recuperar,))
                                        servicio_recuperar = mi_cursor.fetchone()

                                        if servicio_recuperar:
                                            clave_recuperar, nombre_recuperar, costo_recuperar, estado_recuperar = servicio_recuperar
                                            print(f"\nDetalle del servicio a recuperar (Clave: {clave_recuperar}, Nombre: {nombre_recuperar}, Costo: {costo_recuperar})")
                                            confirmar_recuperacion = input("¿Está seguro de recuperar este servicio? (SI/NO): ")

                                            if confirmar_recuperacion.upper() == "SI":
                                                mi_cursor.execute("UPDATE Servicios SET Estado_Servicio = 0 WHERE ClaveS = ?", (clave_recuperar,))
                                                conn.commit()
                                                print(f"El servicio con clave {clave_recuperar} ha sido recuperado.")
                                            else:
                                                print("Recuperación cancelada.")
                                        else:
                                            print("La clave de servicio no se encuentra en el sistema.")
                        ...
                        try:
                            with sqlite3.connect("PIA.db") as conn:
                                ...
                                while True:
                                    ...
                                    if sub_opcion == "1":
                                        mi_cursor.execute("SELECT ClaveS, Nombre FROM Servicios WHERE Estado_Servicio = 0 ORDER BY ClaveS")
                                        servicios = mi_cursor.fetchall()
                                        if not servicios:
                                            print("No se encuentran Servicios registrados o activados")
                                        clave_buscar = input("Ingrese la clave del servicio a consultar: ")
                                        mi_cursor.execute("SELECT ClaveS, Nombre, Costo FROM Servicios WHERE ClaveS = ? AND Estado_Servicio = 0", (clave_buscar,))
                                        ...
                                        mi_cursor.execute("SELECT ClaveS, Nombre, Costo FROM Servicios WHERE UPPER(Nombre) = UPPER(?) AND Estado_Servicio = 0", (nombre_buscar,))
                                        servicios = mi_cursor.fetchall()
                                        ...
                                        if ordenamiento == "1":
                                            ...
                                            mi_cursor.execute("SELECT ClaveS, Nombre, Costo FROM Servicios WHERE Estado_Servicio = 0 ORDER BY ClaveS")
                                            servicios = mi_cursor.fetchall()
                                                    ...
                                                    if exportar.upper() == "A":
                                                        ...
                                                        try:
                                                            ...
                                                            with pd.ExcelWriter(nombre_archivo, engine='xlsxwriter') as writer:
                                                                df.to_excel(writer, sheet_name='Sheet1', index=False)
                                                                worksheet = writer.sheets['Sheet1']
                                                                worksheet.set_column('B:B', len(max(df['Nombre'], key=len)) + 2)
                                                            print(f"Resultado exportado a {nombre_archivo}")
                                                        except Exception as e:
                                                            print(f"Error al exportar a Excel: {str(e)}")
                                                    else:
                                            ...
                                            mi_cursor.execute("SELECT Nombre, ClaveS, Costo FROM Servicios WHERE Estado_Servicio = 0 ORDER BY Nombre")
                                            servicios = mi_cursor.fetchall()
